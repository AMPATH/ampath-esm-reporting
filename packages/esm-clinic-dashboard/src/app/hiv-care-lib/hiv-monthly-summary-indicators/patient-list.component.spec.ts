...
describe('Component: HivSummaryIndicatorsPatientListComponent', () => {
  ...
  it('should render component properties correctly', (done) => {
    ...
    currentTestComponent.startDate = dayjs(new Date());
    currentTestComponent.endDate = dayjs(new Date());
    ...
    expect(date.textContent).toContain(dayjs(new Date()).format('DD/MM/YYYY'));
    expect(date.textContent).toContain(dayjs(new Date()).format('DD/MM/YYYY'));
    done();
  });
});
