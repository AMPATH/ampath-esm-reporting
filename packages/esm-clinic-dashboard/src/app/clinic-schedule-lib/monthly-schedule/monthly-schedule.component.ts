...
export class MonthlyScheduleBaseComponent implements OnInit, OnDestroy {
  public viewDate = dayjs().format('MMMM YYYY');
  public view = 'month';
  public filter: any = {
    programType: [],
    visitType: [],
  ...
  public navigateToMonth() {
    ...
    this.viewDate = dayjs().format('YYYY-MM');
  ...
  public getAppointments() {
    ...
    this.setBusy();
    this.viewDate = dayjs(this.params.startDate, 'YYYY-MM-DD').format(
      'MMMM YYYY'
    );
  ...
  public navigateToDaily(event) {
    const scheduleDate = dayjs(event.start).format('YYYY-MM-DD');
    const params: any = {};
