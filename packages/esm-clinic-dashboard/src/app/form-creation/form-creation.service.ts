    ...
    return Promise.all([
      config.customDataSources.map(async ({ name, moduleName, moduleExport }) => {
        ...
        if (window.hasOwnProperty(slug)) {
          ...
          try {
            const factory: () => Record<string, unknown> = await (moduleEntry as any).get('./start');
            const module = factory();

            if (!(typeof module === 'object') || !module.hasOwnProperty(moduleExport)) {
              if (!silent) {
                console.error(
                  `esm-clinic-dashboard could not load the module ${moduleName} for the datasource ${name} because the module did not have the expected ${moduleExport == 'default' ? 'default export' : `export ${moduleExport}`
                  }.`,
                );
              }
              loadedCustomDataSources[name] = undefined;
            ...
            if (!silent) {
              console.error(
                `esm-clinic-dashboard could not load the datasource ${name} because ${moduleName} did not export the expected entry "./start". Please check the Webpack configuration for ${moduleName}.`,
                e,
              );
            }
            loadedCustomDataSources[name] = undefined;
          ...
          if (!silent) {
            console.error(
              `esm-clinic-dashboard is configured to use the datasource ${name}, but the corresponding module ${moduleName} has not been loaded. Most likely, this means that ${moduleName} is not in your importmap or was not loaded prior to ending up here.`,
            );
          }
          loadedCustomDataSources[name] = undefined;
          return Promise.resolve();
