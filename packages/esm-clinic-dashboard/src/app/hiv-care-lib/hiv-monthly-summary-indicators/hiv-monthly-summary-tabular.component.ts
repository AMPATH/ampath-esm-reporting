...
export class HivSummaryMonthlyTabularComponent implements OnInit {
  ...
  public goToPatientList(data) {
    const endDate = dayjs(data.data.month).endOf('month').format('DD/MM/YYYY');
    const startDate = dayjs(data.data.month)
      .startOf('month')
      .format('DD/MM/YYYY');
    this.locationUuids = data.data.location_uuid;
    this.startDate = dayjs(this._dates.startDate);
    this.endDate = dayjs(this._dates.endDate);
