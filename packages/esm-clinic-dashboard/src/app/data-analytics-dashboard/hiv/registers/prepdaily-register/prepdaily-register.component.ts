...
export class PrepdailyRegisterComponent implements OnInit {
  ...
  public onMonthChange(value): any {
    this._month = dayjs(value).format('MM-DD-YYYY');
  }
  ...
  public storeParamsInUrl() {
    this.params = {
      ...
      month: dayjs(this._month).format('YYYY-MM-DD'),
  ...
  getPrepMethodTotal(data: any[], prepStatus: string, prepStatusValue: string) {
    return (
      data.filter(
        (param) =>
          (param['prep_method'] === '1' ||
