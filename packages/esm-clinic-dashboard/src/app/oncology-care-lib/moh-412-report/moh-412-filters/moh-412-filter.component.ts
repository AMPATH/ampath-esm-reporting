...
export class MOH412FilterComponent implements OnInit, OnChanges {
  @Input() public startDate = dayjs().startOf('month').format('YYYY-MM-DD');
  @Input() public endDate = dayjs().endOf('month').format('YYYY-MM-DD');
    ...
    for (const item of result) {
      ...
      for (const r in data) {
        if (data.hasOwnProperty(r)) {
          const month = dayjs(data.month).format('MMM, YYYY');
  ...
  public onMonthChange($event) {
    this.startDate = dayjs($event).startOf('month').format('YYYY-MM-DD');
    this.endDate = dayjs($event).endOf('month').format('YYYY-MM-DD');
  }
