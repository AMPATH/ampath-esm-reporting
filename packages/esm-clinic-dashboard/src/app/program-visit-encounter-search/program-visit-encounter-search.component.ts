...
export class ProgramVisitEncounterSearchComponent
  ...
  public filterMonth = dayjs().format('YYYY-MM');
  public filterDate = dayjs().format('YYYY-MM-DD');
  ...
  public setParams() {
    ...
    if (this.program.length === 0 && this.showProgramFilters === true) {
      ...
      if (this.dateControl) {
        selectedStartDate = dayjs(this.filterDate).format('YYYY-MM-DD');
        selectedEndDate = dayjs(this.filterDate).format('YYYY-MM-DD');
      }
      if (this.monthControl) {
        selectedStartDate = dayjs(this.filterMonth, 'YYYY-MM')
          .startOf('month')
          .format('YYYY-MM-DD');
        selectedEndDate = dayjs(this.filterMonth, 'YYYY-MM')
          .endOf('month')
          .format('YYYY-MM-DD');
      }
  ...
  public previousMonth() {
    ...
    const prevMonth = dayjs(currentMonth, 'YYYY-MM')
      .subtract(1, 'month')
      .format('YYYY-MM');
  ...
  public nextMonth() {
    const currentMonth = this.filterMonth;
    const nextMonth = dayjs(currentMonth, 'YYYY-MM')
      .add(1, 'month')
      .format('YYYY-MM');
  ...
  public getSelectedDate($event) {
    const newDate: string = dayjs($event).format('YYYY-MM-DD');
    this.filterDate = newDate;
    this.filterSet = false;
  }
  public previousDay() {
    const currentDay = this.filterDate;
    const newDate = dayjs(currentDay).subtract(1, 'day').format('YYYY-MM-DD');
    this.filterDate = newDate;
    this.setFilter();
  }
  public nextDay() {
    const currentDay = this.filterDate;
    const newDate = dayjs(currentDay).add(1, 'day').format('YYYY-MM-DD');
    this.filterDate = newDate;
    this.setFilter();
  }
