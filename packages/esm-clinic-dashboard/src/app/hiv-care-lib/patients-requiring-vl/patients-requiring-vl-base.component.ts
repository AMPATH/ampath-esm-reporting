...
export class PatientsRequiringVLBaseComponent implements OnInit {
  ...
  private _startDate: Date = dayjs().subtract(1, 'months').toDate();
  ...
  public onCurrentVLDateChanged(currentVLDate) {
    if (!currentVLDate) {
      ...
      const filteredPatients = this.data.filter((patient) => {
        return dayjs(patient.current_vl_date, 'DD-MM-YYYY').isBefore(
          dayjs(currentVLDate, 'YYYY-MM-DD')
        );
      });
  ...
  private toDateString(date: Date): string {
    return dayjs(date).utcOffset('+03:00').format();
  }
}
