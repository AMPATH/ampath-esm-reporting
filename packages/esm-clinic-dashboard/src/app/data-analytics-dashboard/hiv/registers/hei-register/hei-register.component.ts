...
export class HeiRegisterComponent implements OnInit {
  ...
  private _startDate: Date = dayjs().startOf('month').toDate();
  ...
  private _endDate: Date = dayjs().endOf('month').toDate();
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((data) => {
      data.month === undefined
        ? (this._month = dayjs().format('YYYY-MM-DD'))
  ...
  public onMonthChange(value): any {
    this._month = dayjs(value).format('YYYY-MM-DD');
  }
  ...
  get formattedStartDate(): string {
    return this.startDate
      ? dayjs(this.startDate).subtract(1, 'year').format('MMM-YYYY')
      : '';
  }

  public storeParamsInUrl() {
    const startDateMinusOneYear = dayjs(this.startDate).subtract(1, 'year');
    const endDateMinusOneYear = dayjs(this.endDate).subtract(1, 'year');
    this.params = {
  ...
  public showDraftReportAlert(date) {
    if (date != null && date >= dayjs().format('YYYY-MM-DD')) {
