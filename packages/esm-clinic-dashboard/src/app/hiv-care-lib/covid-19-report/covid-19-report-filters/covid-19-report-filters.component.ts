...
export class Covid19ReportFiltersComponent implements OnInit, OnChanges {
  ...
  @Input() public startDate: Date = dayjs().endOf('month').toDate();
  ...
  public setParams() {
    this.params = {
      endingMonth: dayjs(this.monthString).endOf('month').format('YYYY-MM-DD'),
      locationUuids: ''
    };
  }
  ...
  public resetFilter() {
    this.monthString = dayjs().endOf('month').format('YYYY-MM');
    this.filteReset.emit(true);
  }
  ...
  public validateFiltersSelected(): Boolean {
    ...
    const filterEndMonth = dayjs(this.monthString).endOf('month');
