...
export class HtsrefferallinkageRegisterComponent implements OnInit {
  ...
  private _startDate: Date = dayjs().toDate();
  ...
  public onMonthChange(value): any {
    this._month = dayjs(value).endOf('month').format('YYYY-MM-DD');
  }
  ...
  public transformApiData(apiData: any[]) {
    return {
      htsLabData: apiData.map((item) => ({
        serialNumber: item.serial_number || 0,
        amrsID: item.amrsID || 0,
        nupiNumber: item.nupi || 0,
        nationalId: item.id_number || 0,
        visitDate: item.date_of_visit
          ? new Date(item.date_of_visit).toISOString().split('T')[0]
          : '',
        clientName: `${item.first_name || 0} ${item.middle_name || 0} ${
          item.last_name || 0
        }`.trim(),
        age: item.age || 0,
        sex: item.sex || 0,
        telephoneNumber: item.telephone || 0,
        maritalStatus: item.marital_status || 0,
        populationType: item.population_type || 0,
        subPopulation: item.sub_population || 0,
        setting: item.setting || 0,
        hivTest1: {
          kitName: item.kit1_name || 0,
          lotNumber: item.lot1 || 0,
          expiryDate: item.expiry1
            ? new Date(item.expiry1).toISOString().split('T')[0]
            : '',
          result: item.hiv_test1 || 0
        },
        hivTest2: {
          kitName: item.kit2_name || 0,
          lotNumber: item.lot2 || 0,
          expiryDate: item.expiry2
            ? new Date(item.expiry2).toISOString().split('T')[0]
            : '',
          result: item.hiv_test2 || 0
        },
        hivTest3: {
          kitName: item.kit3_name || 0,
          lotNumber: item.lot3 || 0,
          expiryDate: item.expiry3
            ? new Date(item.expiry3).toISOString().split('T')[0]
            : '',
          result: item.hiv_test3 || 0
        },
        finalHivResult: item.final_result || 0,
        discordantCouple: '', // Not available in API data
        referredForPrevention: item.referral_services || 0,
        htsProvider: item.hts_provider || 0,
        remarks: item.initial_remarks || 0
      })),
      htsLinkageData: apiData.map((item) => ({
        ...
        identificationStrategy: item.client_tested_as || 0,
        patientReferredTo: item.referral_to || 0,
        handedOverTo: item.handed_to || 0,
        cadre: '', // Not available in API data
        artStartDate: item.art_start_date
          ? new Date(item.art_start_date).toISOString().split('T')[0]
          : '',
        cccNumber: item.ccc_number || 0,
        remarks: item.screening_remarks || 0
      })),
      pinnedBottomRowData: [
        {
  ...
  public showDraftReportAlert(date) {
    if (date != null && date >= dayjs().endOf('month').format('YYYY-MM-DD')) {
  ...
  getTotal(summaryData: any[], gender: string) {
    return summaryData.filter((data) => data['sex'] === gender).length || 0;
  }
  ...
  getPositiveTotal(summaryData: any[]) {
    return (
      summaryData.filter((data) => data['final_result'] === 'P').length || 0
    );
  }
  getTotals(
    summaryData: any[],
