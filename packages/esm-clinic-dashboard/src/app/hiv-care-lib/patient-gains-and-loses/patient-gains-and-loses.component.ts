...
export class PatientGainsAndLosesComponent implements OnInit {
  ...
  public startMonth: Date = dayjs()
    .subtract(2, 'months')
    .endOf('month')
    .toDate();
  public endMonth: Date = dayjs()
    .subtract(1, 'months')
    .endOf('month')
    .toDate();
      ...
      (params: any) => {
        if (params) {
          if (params.startingMonth) {
            this.params = params;
            this.startMonth = dayjs(params.startingMonth).toDate();
            this.endMonth = dayjs(params.endingMonth).toDate();
            this.generateReport();
          }
        }
      },
      ...
      this.patientGainLose
        ...
        .subscribe((data: any) => {
          if (data.error) {
            ...
            this.patientGainAndLoseSummaryData.startingMonth = dayjs(
              this.params.startingMonth
            ).format('MMMM');
            this.patientGainAndLoseSummaryData.endingMonth = dayjs(
  ...
  public onIndicatorSelected($event: any) {
    ...
    this.router.navigate(['patient-list'], {
      ...
      queryParams: {
        ...
        startingMonth: dayjs(this.params.startingMonth)
          .endOf('month')
          .format('YYYY-MM-DD'),
        endingMonth: dayjs(this.params.endingMonth)
          .endOf('month')
          .format('YYYY-MM-DD'),
  ...
  public setParams() {
    this.params = {
      ...
      _date: dayjs(this._month).format('DD-MM-YYYY'),
